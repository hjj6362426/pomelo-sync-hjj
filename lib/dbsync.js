...
DataSync.prototype.recoverDefault = function () {
  ...
  var start = self.config.PrevChipSplit;
  var end = self.config.CurChipSplit;
  if (!start) {
    console.log('recover failed, start split miss');
    self.timer.start(this);
    return;
  }
  self.recover(self.filename, start);
};
...
DataSync.prototype.recover = function (fileName, start, end) {
  ...
  if (strStartSplit === '') isFindStart = true;   // 如果设定起始位置为''，则从文件最开始读入
  if (strEndSplit === '') isFindEnd = true;       // 如果设定结束位置为''，则读到文件结束
