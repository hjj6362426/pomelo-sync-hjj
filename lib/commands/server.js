 ...
 */
 exports.exec = function(){
     //if (this.count == 0) {
     //    console.log('exec time = ' + Date.now());
     //}
    var mergerKey;
    switch (arguments.length) {
        ...
        var isDone = false;
        if (mergerKey.indexOf('Sync.update') >= 0) {
            var searchKey = mergerKey.replace('Sync.update', 'Sync.add');
            if (!!this.mergerMap[searchKey]) {
                //console.log('merge update to add');
                this.mergerMap[searchKey].val = clone(arguments[2]);
                isDone = true;
            }
        }
        if(!isDone) {
            ...
            //++this.count;
        this.writeToAOF(arguments[0], [arguments[1], arguments[2]]);
        break;
        default:
        break;
    }
